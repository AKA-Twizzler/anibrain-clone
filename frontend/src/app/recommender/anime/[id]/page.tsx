...
export default function AnimeDetailPage() {
  ...
  const { data: media, error: mediaError, isLoading: mediaLoading } = useMedia(`Anime/${id}`);
  const { data: recommendations, error: recError, isLoading: recLoading } = useRecommendations(`Anime/${id}`);
  ...
  if (mediaError || !media) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <p className="text-error text-lg font-medium mb-2">Failed to load anime</p>
        <p className="text-text-muted text-sm mb-6">{mediaError?.message || 'Anime not found'}</p>
        <Link
  ...
  const bannerUrl = media.bannerImage || api.buildBannerUrl(media.anilistId, 'ANIME');
  const coverUrl = media.coverImage || api.buildImageUrl(media.anilistId, 'ANIME');
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 pt-4">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              {media.episodes && (
                <span className="text-sm text-text-muted">{media.episodes} Episodes</span>
              )}
              {media.duration && (
                <span className="text-sm text-text-muted">{media.duration} min/ep</span>
              )}
              {media.averageScore && (
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              ...
              {media.trailer && (
                <a
                  ...
                  className="px-5 py-2.5 rounded-xl bg-bg-card border border-border-secondary text-text-primary text-sm font-medium hover:border-accent/50 transition-colors inline-flex items-center gap-2"
                >
        ...
        <div className="mt-16 pb-16">
          ...
          {recError && (
            <div className="text-center py-12 bg-bg-card rounded-2xl border border-border-secondary">
              <p className="text-text-muted">Unable to load recommendations</p>
            </div>
          )}
