...
export default function LightNovelToOneShotPage() {
  const params = useParams();
  const id = params.id as string;

  const { data, error, isLoading } = useCrossCategory('LIGHT_NOVEL', 'ONE_SHOT', id);
  ...
  const coverUrl = sourceMedia.coverImage || api.buildImageUrl(sourceMedia.anilistId, 'LIGHT_NOVEL');
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10">
        <div className="flex flex-col md:flex-row gap-6 items-start mb-12">
          ...
          <div className="flex-1 pt-4">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              {sourceMedia.chapters && (
                <span className="text-sm text-text-muted">{sourceMedia.chapters} Chapters</span>
              )}
            </div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/10 border border-accent/20">
              <span className="text-xs font-semibold text-accent">Light Novel to One Shot</span>
            </div>
          </div>
        </div>
        ...
        <div className="pb-16">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-text-primary">One Shot Manga for {title} Fans</h2>
            <p className="text-text-muted mt-1 text-sm">
              Discover one-shot manga recommendations based on this light novel
            </p>
          </div>
          ...
          {recommendations && recommendations.length > 0 ? (
            ...
            <div className="text-center py-12 bg-bg-card rounded-2xl border border-border-secondary">
              <p className="text-text-muted">No one-shot recommendations available for this light novel yet.</p>
            </div>
          )}
        </div>
      </div>
