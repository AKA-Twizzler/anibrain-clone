...
export default function OneShotToMangaPage() {
  const params = useParams();
  const id = params.id as string;

  const { data, error, isLoading } = useCrossCategory('ONE_SHOT', 'MANGA', id);
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start mb-12">
          ...
          <div className="flex-1 pt-4">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/10 border border-accent/20">
              <span className="text-xs font-semibold text-accent">One Shot to Manga</span>
            </div>
          </div>
        </div>
        ...
        <div className="pb-16">
          <div className="mb-8">
            ...
            <p className="text-text-muted mt-1 text-sm">
              Discover manga recommendations based on this one shot
            </p>
          </div>
          ...
          {recommendations && recommendations.length > 0 ? (
            ...
            <div className="text-center py-12 bg-bg-card rounded-2xl border border-border-secondary">
              <p className="text-text-muted">No manga recommendations available for this one shot yet.</p>
            </div>
          )}
        </div>
      </div>
